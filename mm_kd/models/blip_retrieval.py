...
##==================KD==============================
        ...
        onetower_list = []
        with torch.no_grad():
            for i in range(len(encoder_input_ids_chunk)):
                output_pos_t = self.text_encoder(encoder_input_ids_chunk[i],
                                            attention_mask = attention_mask_chunk[i],
                                            encoder_hidden_states = image_embeds_chunk[i],
                                            encoder_attention_mask = image_atts_chunk[i],      
                                            return_dict = True,
                                            )  
                onetower_list.append(F.softmax(self.itm_head(output_pos_t.last_hidden_state[:,0,:]), dim=1)[:, 1].reshape(1, encoder_input_ids.shape[0]))
        ranker_score = torch.cat(onetower_list, 0)   
        ranker_score = ranker_score #- torch.mean(ranker_score, dim=1).unsqueeze(1)                     
        ...
        # add colbert loss
        image_feat_colbert = F.normalize(self.vision_proj(image_embeds), dim=-1)
        text_feat_colbert = F.normalize(self.text_proj(text_output.last_hidden_state), dim=-1)
